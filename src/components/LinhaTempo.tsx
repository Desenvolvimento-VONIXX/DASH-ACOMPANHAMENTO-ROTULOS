...
const LinhaTempo: React.FC<Props> = ({ idClick, linha_tempo, setOpenVerChecklist, setIdAtividade }) => {

    console.log("Clicked on", idClick);
    console.log("Clicked on", linha_tempo);
                ...
                {linha_tempo.map((item: LinhaTempo) => {
                    return (
                        <li key={item.ID_ATIVIDADE} className="mb-10 ms-4">
                            ...
                            {item.STATUS === "ENVIADO" && (
                                <Button onClick={() => handleBaixaAnexo(item.ID_ATIVIDADE)} className="mt-2 inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-400 rounded-lg hover:bg-gray-100 ">
                            ...
                            {item.STATUS === "FINALIZADO" && (
                                <>
                                    <p className="text-base font-normal text-gray-500 dark:text-gray-400"><span className="font-semibold">Justicativa: </span>{item.JUSTIFICATIVA}</p>
                                    <Button onClick={() => handleBaixaAnexo(item.ID_ATIVIDADE)} className="mt-2 inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-400 rounded-lg hover:bg-gray-100 ">
                                        <svg className="w-3.5 h-3.5 me-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M14.707 7.793a1 1 0 0 0-1.414 0L11 10.086V1.5a1 1 0 0 0-2 0v8.586L6.707 7.793a1 1 0 1 0-1.414 1.414l4 4a1 1 0 0 0 1.416 0l4-4a1 1 0 0 0-.002-1.414Z" />
                                            <path d="M18 12h-2.55l-2.975 2.975a3.5 3.5 0 0 1-4.95 0L4.55 12H2a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-4a2 2 0 0 0-2-2Zm-3 5a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z" />
                            ...
                            {["APROVADO", "REPROVADO"].includes(item.STATUS) && (
                                <div className="flex gap-4 mt-2">
                                    <Button
                                        ...
                                        className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-400 rounded-lg hover:bg-gray-100 ">
