...
import { useGetRespostasCheck } from "@/hooks/getRespostasChecklist";
...
const VerChecklist: React.FC<Props> = ({ open, onClose, idClick, idAtividade }) => {
    ...
    const porcentagens = respostas ? calcularPorcentagens(respostas) : { CF: 0, NC: 0, NA: 0 };
    ...
    const exportarParaPDF = () => {
        ...
        doc.addImage("/Imagem1.jpg", "JPEG", centerX, startY, imageWidth, imageHeight);
        ...
        if (!respostas || respostas.length === 0) {
            ...
            doc.text(`Página ${pageNumber}`, pageWidth - 30, doc.internal.pageSize.height - 10);
        ...
        doc.text(`Data da realização: ${respostas[0].DATA_FORMATADA}`, marginLeft, startY);
        startY += 6;

        doc.text(`Responsável pela Verificação: ${respostas[0].NOMEUSU}`, marginLeft, startY);
        startY += 10;

        respostas.forEach((item: Resposta, index: number) => {
            doc.setFont("helvetica", "bold");
            const splitPergunta = doc.splitTextToSize(`${index + 1}. ${item.PERGUNTA}`, pageWidth - 20);

            if (startY + splitPergunta.length * 6 > doc.internal.pageSize.height - 20) {
                doc.addPage();
                pageNumber++;
                startY = 20;
            }
            ...
            if (item.OBS) {
                ...
                const splitObs = doc.splitTextToSize(`Observação: ${item.OBS}`, pageWidth - 30);
    ...
    return (
        <Sheet open={open} onOpenChange={onClose}>
            <SheetContent className="min-w-[50%]">
                <SheetHeader>
                    <SheetTitle className="text-2xl font-semibold text-gray-800 mb-3">Avaliação do Rótulo</SheetTitle>
                </SheetHeader>
                    ...
                    {respostas?.length > 0 ? (
                        <div className="p-5 rounded-sm">
                            ...
                            <div className="bg-gray-200 p-4 rounded-md mb-6">
                                ...
                                <Button onClick={exportarParaPDF} className="mt-2 w-full flex items-center gap-1" variant={"outline"}>
                            ...
                            {respostas.map((item: any, index: number) => (
                                <div key={index} className="mb-4 p-4 rounded-lg bg-white shadow-md border border-gray-300">
                                ...
                                <Table className="min-w-full table-auto bg-white rounded-lg shadow-sm overflow-hidden">
                                    <TableHeader>
                                        <TableRow className="bg-gray-100 text-gray-900">
                                            <TableHead className="py-3 px-4 text-left text-sm font-semibold">Opção</TableHead>
                                            <TableHead className="py-3 px-4 text-left text-sm font-semibold">Porcentagem (%)</TableHead>
                                        </TableRow>
                                    </TableHeader>

                                    <TableBody>
                                        <TableRow>
                                            <TableCell className="py-3 px-4 text-sm text-green-700 border-t border-b ">CF</TableCell>
                                            <TableCell className="py-3 px-4 text-sm text-gray-800 border-t border-b font-semibold">{porcentagens.CF.toFixed(2)}%</TableCell>
                                        </TableRow>
                                        <TableRow>
                                            <TableCell className="py-3 px-4 text-sm text-red-700 border-t border-b">NC</TableCell>
                                            <TableCell className="py-3 px-4 text-sm text-gray-800 border-t border-b font-semibold">{porcentagens.NC.toFixed(2)}%</TableCell>
                                        </TableRow>
                                        <TableRow>
                                            <TableCell className="py-3 px-4 text-sm text-yellow-700 border-t border-b">NA</TableCell>
                                            <TableCell className="py-3 px-4 text-sm text-gray-800 border-t border-b font-semibold">{porcentagens.NA.toFixed(2)}%</TableCell>
                                        </TableRow>
                                    </TableBody>
                                </Table>
                            </div>
                        ...
                        <p className="text-center text-gray-500 mt-5">Nenhuma resposta encontrada.</p>
                    )}
                </ScrollArea>
            </SheetContent>
        </Sheet>
